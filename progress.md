...
Dosya	Açıklama
server.js
Ana backend — tüm API endpointleri burada
package.json
Bağımlılıklar: express, yt-dlp-exec, axios, ioredis, p-queue
config.json
Global ayarlar (enabled, mode)
Dockerfile
Docker imajı (node:20 + python + ffmpeg)
cookies.txt
YouTube cookie dosyası (yt-dlp için)
blockedChannels.json
Engelli kanal listesi (şu an boş)
🔌 API Endpointleri
...
cookies.txt
 dosyası var ama yt-dlp çağrılarında kullanılmıyor
...
/stream endpointi cache kullanmıyor (her seferinde yt-dlp çağırıyor)
...
1.5	Proxy desteği ekle (residential proxy rotasyonu)	⏭️ (Atlandı)
1.6	Hardcoded secret'ları .env dosyasına taşı	✅
1.7	Stream/download endpointlerine auth ekle	⏭️ (Android uyumluluk)
...
2.4	Stream URL'lerini önceden çözümleme (pre-resolve) — top50'deki videolar için	⏭️ (Opsiyonel)
2.5	Search cache süresini config'den yönet	✅
2.6	
blockedChannels.json
 filtrelemesini implement et	✅
